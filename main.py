...
def complete_order(parameters: dict, session_id: str):
    if session_id not in inprogress_orders:
         fulfillmentText = "I'm having some trouble finding your order. Sorry! Can you place a new order please?"
    else:
        order = inprogress_orders[session_id]
        order_id = save_to_db(order)

        if order_id == -1:
            fulfillmentText = "Sorry, I couldn't process your order due to backend error."\
...
def add_to_order(parameters: dict, session_id: str):
    ...
    if len(food_items) != len(quantities):
        fulfillmentText = "Sorry I didn't understand. Can you please specify food items and quantity?"
    else:
...
def remove_from_order(parameters: dict, session_id: str):
    ...
    if len(no_such_items) > 0:
        fulfillmentText = f'Your current order does not have{", ".join(no_such_items)} items'
